document.addEventListener("DOMContentLoaded", function() { // Espera a que la página cargue completamente antes de ejecutar el script
  ...
  // Selecciona todos los tooltips (contenedores) 
  const tooltips = document.querySelectorAll(".tooltip"); // Selecciona todos los tooltips (contenedores)

  // Muestra el contenedor (tooltip) cuando pasas el ratón sobre el texto resaltado
  highlight.forEach((text, index) => {
    text.addEventListener("mouseover", () => {
      tooltips[index].style.display = "block"; // Muestra el contenedor (tooltip) correspondiente
    });

    // Oculta el contenedor (tooltip) cuando el ratón sale del texto
    text.addEventListener("mouseout", () => {
      tooltips[index].style.display = "none"; // Oculta el contenedor (tooltip)
    });
  });
});